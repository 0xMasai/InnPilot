...
import {
  AreaChart,
  Area,
  ResponsiveContainer,TEST_LINE_1
const x = { a: [1,2], b: `hi ${1+1}` };
TEST_LINE_ENDaimport { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Link } from "react-router-dom";
import {
  AreaChart,
...
/* ------------------------------------------------------------
   WebMCP tools — the AI-agent surface
   Mirrors src/webmcp/tools/index.ts (5 read + 3 write).
   ------------------------------------------------------------ */
const webmcpTools: {
  name: string;
  group: "Read" | "Write";
  ...
  { name: "innpilot_list_rooms", group: "Read", icon: <Bed size={18} />, description: "Room inventory — number, type, nightly rate and housekeeping status." },
  { name: "innpilot_list_reservations", group: "Read", icon: <CalendarDays size={18} />, description: "Current reservations, optionally filtered by status." },
  { name: "innpilot_check_room_availability", group: "Read", icon: <DoorOpen size={18} />, description: "Rooms free for a stay, using the exact conflict rule that guards booking." },
  { name: "innpilot_get_occupancy", group: "Read", icon: <Percent size={18} />, description: "Occupancy rate, with occupied and available room counts." },
  { name: "innpilot_get_revenue", group: "Read", icon: <Banknote size={18} />, description: "Revenue by department, expenses and net operating result." },
  { name: "innpilot_create_reservation", group: "Write", icon: <CalendarPlus size={18} />, description: "Create a confirmed reservation, rejecting conflicts and maintenance holds." },
  { name: "innpilot_update_reservation_status", group: "Write", icon: <LogIn size={18} />, description: "Check in or out, cancel, or mark a reservation as no-show." },
  { name: "innpilot_set_room_status", group: "Write", icon: <Wrench size={18} />, description: "Set a room's housekeeping or maintenance status." },
];
...
const faqs: { q: string; a: string }[] = [
  ...
  { q: "Can an AI agent operate InnPilot?", a: "Yes. InnPilot exposes its real operating capabilities as WebMCP tools on document.modelContext. In a WebMCP-capable browser, an AI agent in the signed-in session can call the same eight tools your team uses — bound by the same role, tenant and audit trail. In any other browser the tools simply don't register." },
...
/* ------------------------------------------------------------
   WebMCP tool call mock (agent → tool → structured result)
   ------------------------------------------------------------ */
const WebMCPCallMock = () => (
  <div className="rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-[#0b1220]">
    <div className="flex items-center gap-1.5 px-4 py-3 border-b border-white/10">
      <span className="w-2.5 h-2.5 rounded-full bg-white/20" />
      <span className="w-2.5 h-2.5 rounded-full bg-white/20" />
      <span className="w-2.5 h-2.5 rounded-full bg-white/20" />
      <span className="ml-3 text-[11px] text-[#8b97ad]">agent · document.modelContext</span>
    </div>
    <div className="p-4 sm:p-5 space-y-3 font-mono text-[12px] leading-relaxed">
      <div className="text-[#8b97ad]">
        <span className="text-[var(--brand-cyan)]">agent</span> ▸ "Which rooms are free tomorrow night?"
      </div>
      <div className="text-white">
        → call <span className="text-[var(--brand-cyan)]">innpilot_check_room_availability</span>
      </div>
      <pre className="text-[#a7b3c9] whitespace-pre-wrap bg-white/5 rounded-lg p-3 border border-white/10 m-0">{`{
  "checkIn": "2025-09-04",
  "checkOut": "2025-09-05"
}`}</pre>
      <div className="text-white">← result</div>
      <pre className="text-[#7ee0d3] whitespace-pre-wrap bg-white/5 rounded-lg p-3 border border-white/10 m-0">{`3 rooms available:
  101 · Standard · UGX 120,000
  104 · Deluxe   · UGX 180,000
  110 · Suite    · UGX 260,000`}</pre>
    </div>
  </div>
);
...
const LandingPage = () => {
  return (
    <div className="min-h-screen w-full font-poppins text-slate-800" style={{ background: "var(--app-bg)" }}>
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 border-b border-slate-200">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-3">
          ...
          <div className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-500">
            ...
            <a href="#webmcp" className="hover:text-slate-900 transition">For AI agents</a>
      ...
      <section className="relative overflow-hidden" style={{ background: "var(--rail)" }}>
        ...
        <div className="relative max-w-6xl mx-auto px-6 pt-20 pb-24 grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial="hidden" animate="show" variants={fadeUp}>
            ...
            <div className="mt-6 inline-flex items-center gap-2 text-xs font-medium text-[var(--rail-text)]">
              <Bot size={14} className="text-[var(--brand-cyan)]" />
              Now operable by AI agents over WebMCP
              <a href="#webmcp" className="text-[var(--brand-cyan)] hover:underline">See the tools →</a>
            </div>
          </motion.div>
      ...
      {/* WebMCP — the AI-agent surface */}
      <section id="webmcp" className="relative overflow-hidden py-20" style={{ background: "var(--rail)" }}>
        <div
          className="pointer-events-none absolute -top-24 right-0 w-[380px] h-[380px] rounded-full blur-3xl"
          style={{ background: "radial-gradient(circle, rgba(45,212,200,0.18) 0%, transparent 70%)" }}
        />
        <div className="relative max-w-6xl mx-auto px-6">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeUp} className="max-w-2xl mb-10">
            <span className="inline-flex items-center gap-2 text-xs font-semibold tracking-wide uppercase text-[var(--brand-cyan)] mb-4">
              <Globe size={14} /> Powered by WebMCP
            </span>
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3">Your hotel, operable by AI agents</h2>
            <p className="text-[var(--rail-text)] leading-relaxed">
              InnPilot registers its real operating capabilities as WebMCP tools on
              <code className="mx-1 px-1.5 py-0.5 rounded bg-white/10 text-[var(--brand-cyan)] font-mono text-[13px]">document.modelContext</code>.
              In a WebMCP-capable browser, an AI agent in the signed-in session discovers and calls
              these eight tools directly — the same services, permissions and audit trail as your
              team. No screen-scraping, no separate integration.
            </p>
          </motion.div>
          ...
          <div className="grid lg:grid-cols-5 gap-6 items-start">
            ...
            <div className="lg:col-span-3 grid sm:grid-cols-2 gap-3">
              {webmcpTools.map((t, i) => (
                <motion.div
                  ...
                  className="rounded-xl p-4 bg-white/5 border border-white/10 hover:border-[var(--brand-cyan)]/40 transition"
                >
                  <div className="flex items-center justify-between mb-2">
                    <div
                      className="w-9 h-9 rounded-lg flex items-center justify-center"
                      style={{ background: "rgba(45,212,200,0.14)", color: "var(--brand-cyan)" }}
                    >
                      {t.icon}
                    </div>
                    <span
                      className={`text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full ${
                        t.group === "Read"
                          ? "bg-[rgba(45,212,200,0.14)] text-[var(--brand-cyan)]"
                  ...
                  <code className="block font-mono text-[12.5px] text-white mb-1 break-all">{t.name}</code>
                  <p className="text-[13px] text-[var(--rail-text)] leading-relaxed">{t.description}</p>
                </motion.div>
              ))}
            </div>

            {/* Call mock + guarantees */}
            <motion.div
              initial="hidden"
              whileInView="show"
              viewport={{ once: true }}
            ...
            >
              ...
              <ul className="space-y-2.5">
                {[
                  "Runs inside the manager's already-authenticated session",
                  "Role + tenant re-checked on every single call",
                  "Every write is recorded in the audit log as an agent action",
                ].map((item) => (
                  <li key={item} className="flex items-start gap-2 text-sm text-[var(--rail-text)]">
                    <ShieldCheck size={16} className="mt-0.5 shrink-0 text-[var(--brand-cyan)]" />
                    {item}
                  </li>
                ))}
              </ul>
            ...
            <div className="rounded-2xl p-8 text-center" style={{ background: "var(--rail-2)" }}>
              ...
              <ul className="text-left space-y-2.5 mb-8">
                {[
                  ...
                  "WebMCP tools for AI agents included",
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-slate-500">
                ...
                <li><a href="#webmcp" className="hover:text-slate-900 transition">For AI agents</a></li>
